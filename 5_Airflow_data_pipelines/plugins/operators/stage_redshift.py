...
class StageToRedshiftOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        self.log.info(f"Getting file for table {self.table_name} from location : {s3_path}")

        if self.log_json_file != "":
            self.log_json_file = "s3://{}/{}".format(self.s3_bucket, self.log_json_file)
            copy_query = self.copy_query.format(self.table_name, s3_path, credentials.access_key,
